...
## 表記の既定ルール（全執筆プロジェクト共通）
プロジェクトの style-guide.md に別段の定義がない限り、以下を既定とする。
- 全角文字と半角数字が隣り合う箇所には半角スペースを入れる（例: `3 コマ`、`1 ページ`、`1 日`）
- 対象外: コード・コマンド・URL・ファイル名の中は原文のまま（例: `settings.json`、`git log -3`）
